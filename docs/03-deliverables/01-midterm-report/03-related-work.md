...
prior to deep learning, mfcc features combined with svm classifiers were the standard approach for ser. on ravdess, mfcc + svm baselines typically achieve 50-65% accuracy depending on the emotion subset, feature configuration, and split strategy [1]. these results establish the performance floor our cnn must exceed. our project uses the same mfcc features for the svm baseline, ensuring a fair comparison.

## reference implementations

open-source ser implementations include emotion-recognition-english [5] and speech_emotion [6]. these typically use 2d cnns on mel spectrogram images and are not optimized for inference speed. our work differs by targeting a compact 1d architecture with explicit cpu deployment constraints.

## gap in the literature

no published work specifically compares a 1d cnn on raw mel spectrograms against an mfcc + svm baseline on ravdess with the explicit goal of cpu-optimized inference, nor investigates cross-channel (speech to song) generalization. this project fills that gap.
